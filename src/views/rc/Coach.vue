<template>
  <div class="coach">
    <div class="content">
      ...
      <div class="wrapper">
        <el-form :model="coach" :rules="rules" ref="coach" label-width="100px" class="demo-ruleForm" @keyup.enter.native="onSubmitCert('coach')">
          <el-form-item label="项目名称" prop="item">
            <el-input v-model="coach.item" placeholder="请输入项目名称"></el-input>
          </el-form-item>
          <el-form-item label="等级" prop="level">
            <el-select v-model="coach.level" placeholder="请选择您的级别">
              <el-option v-for="(item,index) in coachGradeColumns" :key="index" :label="item"
                         :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="证书编号" prop="certCode">
            <el-input v-model="coach.certCode" placeholder="请输入证书编号"></el-input>
          </el-form-item>
          <el-form-item label="核发机构" prop="certSource">
            <el-input v-model="coach.certSource" placeholder="请输入核发机构"></el-input>
          </el-form-item>
          <el-form-item label="证书日期" prop="certTime">
            <el-col :span="11">
              <el-date-picker type="date" placeholder="选择证书日期" v-model="coach.certTime" style="width: 100%;"
...
export default {
      ...
      if (res.code == 200) {
        ...
        if (res.data.userType == 1) {
          ...
          if (res.data.coachInfo.length > 0) {
            self.coach = res.data.coachInfo[res.data.coachInfo.length-1];
  ...
  methods: {
    ...
    handleChange(value) {
      ...
      self.coach.province = provinceCode.concat(cityCode,countyCode)
      // self.coach.city = cityCode
      // self.coach.county = countyCode
    },
    //上传头像
    handleAvatarSuccessCoach(res, file) {
      this.coach.certImg = URL.createObjectURL(file.raw);
      let self = this
    ...
    onSubmitCert(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let self = this;
          self.coach.resumes =self.certResumeList
          let parms = self.coach
