<template>
  <div class="referee">
    <div class="content">
      ...
      <div class="wrapper">
        <el-form :model="referee" :rules="rules" ref="referee" label-width="100px" class="demo-ruleForm" @keyup.enter.native="onSubmitCert('referee')">
          <el-form-item label="项目名称" prop="item">
            <el-input v-model="referee.item" placeholder="请输入项目名称"></el-input>
          </el-form-item>
          <el-form-item label="等级" prop="level">
            <el-select v-model="referee.level" placeholder="请选择您的级别">
              <el-option v-for="(item,index) in refereeGradeColumns" :key="index" :label="item"
                         :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="证书编号" prop="certCode">
            <el-input v-model="referee.certCode" placeholder="请输入证书编号"></el-input>
          </el-form-item>
          <el-form-item label="核发机构" prop="certSource">
            <el-input v-model="referee.certSource" placeholder="请输入核发机构"></el-input>
          </el-form-item>
          <el-form-item label="证书日期" prop="certTime">
            <el-col :span="11">
              <el-date-picker type="date" placeholder="选择证书日期" v-model="referee.certTime" style="width: 100%;"
          ...
          <el-form-item label="证书上传" class="upload" prop="certImg">
            <el-upload
                v-model="referee.certImg"
                class="avatar-uploader"
                action="http://wushu.sportsit.cn:8080/upload/uploadImage"
                :show-file-list="false"
                :on-success="handleAvatarSuccessreferee"
            >
              <img v-if="referee.certImg" :src="referee.certImgs[0].url" class="avatar">
...
export default {
      ...
      if (res.code == 200) {
        ...
        if (res.data.userType == 1) {
          ...
          if (res.data.refereeInfo.length > 0) {
            self.referee = res.data.refereeInfo[res.data.refereeInfo.length-1];
  ...
  methods: {
    ...
    handleChange(value) {
      ...
      self.referee.province = provinceCode.concat(cityCode,countyCode)
      // self.coach.city = cityCode
      // self.coach.county = countyCode
    },
    //上传头像
    handleAvatarSuccessreferee(res, file) {
      this.referee.certImg = URL.createObjectURL(file.raw);
      let self = this
    ...
    onSubmitCert(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let self = this;
          self.referee.certResume = self.certResumeList.toString()
          let parms = self.referee
          ...
          setTimeout(() => {
            if (parms.certImgs.length == 0) {
              ...
              self.$message("请填写项目名称！");
              return false
            } else if (parms.certSource == '') {
              self.$message("请填写核发机构！");
              return false
            } else if (parms.level == '') {
              self.$message("等级不能为空！");
              return false
            } else if (parms.certCode == '') {
              self.$message("请填写证书编号！");
              return false
